...
export class TaskService implements ITaskService {
  ...
  create(request: CreateTaskRequest): Observable<Task> {
    return this.http
      ...
      .pipe(switchMap(taskId => this.getById(taskId)));
  }

  update(id: string, request: UpdateTaskRequest): Observable<Task> {
    return this.http.put(`${this.baseUrl}/${id}`, request).pipe(switchMap(() => this.getById(id)));
  }
    ...
    const cleanRequest = {
      ...
      filters: request.filters
        ? {
            ...
            ...(request.filters.isOverdue !== undefined && {
              isOverdue: request.filters.isOverdue,
            }),
            ...(request.filters.isCompleted !== undefined && {
              isCompleted: request.filters.isCompleted,
            }),
          }
        : undefined,
    };
...
export class TaskRepository {
  ...
  getTasksForDashboard(): Observable<Task[]> {
    ...
    return this.taskService
      ...
      .pipe(map((response: TaskListResponse) => response.items));
  }
}
